...
class ResNet(nn.Module):
    ...
    def __init__(self, args, block, num_block, num_classes=10):
        ...
        if(args.g == 3):
            ...
            self.feature_list[1].append( torch.nn.Sequential(*(list(self.conv4_x)+list(self.conv5_x))).to('cuda:1'))
            self.feature_list[2].append( torch.nn.Sequential(*(list(self.avg_pool))).to('cuda:2'))
            self.fc = self.fc.to('cuda:2')
        if(args.g == 4):
            self.feature_list[0].append( torch.nn.Sequential(*(list(self.conv1)+list(self.conv2_x))).to('cuda:0'))
            self.feature_list[1].append( torch.nn.Sequential(*(list(self.conv3_x)+list(self.conv4_x))).to('cuda:1'))
            self.feature_list[2].append( torch.nn.Sequential(*(list(self.conv5_x))).to('cuda:2'))
            self.feature_list[3].append( torch.nn.Sequential(*(list(self.avg_pool))).to('cuda:3'))
            self.fc = self.fc.to('cuda:3')
        if(args.g == 5):
            self.feature_list[0].append( torch.nn.Sequential(*(list(self.conv1)+list(self.conv2_x))).to('cuda:0'))
            self.feature_list[1].append( torch.nn.Sequential(*(list(self.conv3_x))).to('cuda:1'))
            self.feature_list[2].append( torch.nn.Sequential(*(list(self.conv4_x))).to('cuda:2'))
            self.feature_list[3].append( torch.nn.Sequential(*(list(self.conv5_x))).to('cuda:3'))
            self.feature_list[4].append( torch.nn.Sequential(*(list(self.avg_pool))).to('cuda:4'))
            self.fc = self.fc.to('cuda:4')
        if(args.g == 6):
            self.feature_list[0].append( torch.nn.Sequential(*(list(self.conv1))).to('cuda:0'))
            self.feature_list[0].append( torch.nn.Sequential(*(list(self.conv2_x))).to('cuda:1'))
            self.feature_list[1].append( torch.nn.Sequential(*(list(self.conv3_x))).to('cuda:2'))
            self.feature_list[2].append( torch.nn.Sequential(*(list(self.conv4_x))).to('cuda:3'))
            self.feature_list[3].append( torch.nn.Sequential(*(list(self.conv5_x))).to('cuda:4'))
            self.feature_list[4].append( torch.nn.Sequential(*(list(self.avg_pool))).to('cuda:5'))
